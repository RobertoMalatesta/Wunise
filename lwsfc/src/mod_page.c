#include "../include/mod_page.h"
#include "../include/mod_time.h"
#include "../version.h"
#include <sys/socket.h>
void page404(int client)
{
  send_date(client);
  send(client,SERVERVERSION,sizeof(SERVERVERSION) - 1,0);
  send(client,"Content-Length: 16\r\n",sizeof"Content-Length: 16\r\n" - 1,0);
  send(client,"Content-Type: text/html;charset=UTF-8\r\n",sizeof"Content-Type: text/html;charset=UTF-8\r\n" - 1,0);
  send(client,"Pragma: no-cache\r\n\r\n",sizeof"Pragma: no-cache\r\n\r\n" - 1, 0);
  send(client,"<body>404</body>",sizeof"<body>404</body>" - 1,0);
}
void unimplemented_page(int client)
{
  send_date(client);
  send(client,SERVERVERSION,sizeof(SERVERVERSION) - 1,0);
  send(client,"Allow: GET HEAD OPTIONS\r\n",sizeof"Allow: GET HEAD OPTIONS\r\n" - 1,0);
  send(client,"Content-Length: 33\r\n",sizeof"Content-Length: 33\r\n" - 1,0);
  send(client,"Content-Type: text/html;charset=UTF-8\r\n",sizeof"Content-Type: text/html;charset=UTF-8\r\n" - 1,0);
  send(client,"Pragma: no-cache\r\n\r\n",sizeof"Pragma: no-cache\r\n\r\n" - 1, 0);
  send(client,"<body>unimplemented method</body>",sizeof"<body>unimplemented method</body>" - 1,0);
}
